...
public class _ContentController extends JBaseCRUDController<Content> {
	...
	private void filterUI(BigInteger[] tids) {
		...
		if (types != null && !types.isEmpty()) {
			...
			System.out.println("-------------+++++++++++++++++++++++++++++++++_taxonomyMap:"+_taxonomyMap);
	...
	public void edit() {
		...
		List<TplTaxonomyType> llist = module.getTaxonomyTypes();
		for(int ic = 0; ic < llist.size();ic++){
			TplTaxonomyType tpl = llist.get(ic);
			String tplTitle = tpl.getTitle();
		}
	...
	public void save() {
		...
		if (dbContent != null && content.getId() != null && dbContent.getId().compareTo(content.getId()) != 0) {
			renderAjaxResultForError("更新文章出错!");
			return;
		}
...
/*		System.out.println("----------------------------++++++++++id:"+content.getId());
		...
		if(dbContent != null){
			System.out.println("----------------------------++++++++++slug:"+dbContent.getSlug());
			System.out.println("----------------------------++++++++++title:"+dbContent.getTitle());
			System.out.println("----------------------------++++++++++id:"+dbContent.getId());
		}*/

		if(dbContent != null && content.getId() == null && dbContent.getSlug() != null){
			renderAjaxResultForError("文章唯一slug与存在，请修改文章标题! 请刷新页面后再次编辑!");
			return;
		}
